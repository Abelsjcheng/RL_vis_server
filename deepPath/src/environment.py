...
class KGEnvironment(object):
    ...
    def __init__(self, dataPath, task=None):
        f1 = open(os.path.join(settings.BASE_DIR, 'deepPath/' + dataPath + '/entity2id.txt'))
        f2 = open(os.path.join(settings.BASE_DIR, 'deepPath/' + dataPath + '/relation2id.txt'))
        ...
        f = open(os.path.join(settings.BASE_DIR, 'deepPath/' + dataPath + '/kb_env_rl.txt'))
        kb_all = f.readlines()
        f.close()
