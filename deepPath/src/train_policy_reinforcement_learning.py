...
from deepPath.src.networks import PolicyNN, ValueNN
from deepPath.src.utils import *
from deepPath.src.environment import KGEnvironment
from train_policy_supervised_learning import SupervisedPolicy
# hyperparameters
state_dim = 200
action_space = 400
eps_start = 1
...
dataPath = '../NELL-995/'
model_dir = '../models/'
model_name = 'policy_retrained'

relation = sys.argv[1]
task = sys.argv[2]
graphpath = os.path.join(dataPath, 'tasks', relation, 'graph.txt')
relationPath = os.path.join(dataPath, 'tasks', relation, 'train_pos.txt')
...
def REINFORCE(training_pairs, policy_network, num_episodes):
    ...
    for i_episode in range(num_episodes):
        ...
        # Discourage the agent when it choose an invalid step 当代理选择无效步骤时，请阻止它
        if len(state_batch_negative) != 0:
            print('Penalty to invalid steps:', len(state_batch_negative))
            policy_network.optimizer.zero_grad()
            state_batch_negative = np.reshape(state_batch_negative, (-1, state_dim))
        ...
        if done == 1:
            ...
            loss = policy_network.compute_loss(predictions, total_reward, action_batch)
            loss.backward()
            policy_network.optimizer.step()
        else:
            global_reward = -0.05
            ...
            try:
                ...
                for item in good_episodes:
                    ...
                    loss = policy_network.compute_loss(predictions, 1, teacher_action_batch)
                    loss.backward()
                    policy_network.optimizer.step()
    ...
    relation_path_stats = collections.Counter(path_relation_found).items()  # 正确路径的数量作为权重
...
def retrain():
    ...
    # f.close()
    policy_network.load_state_dict(torch.load('../models/policy_supervised_' + relation + '.pt'))
    policy_network.cuda()
...
def test():
    ...
    policy_network = torch.load('../models/policy_retrained' + relation + '.pt')
    ...
    for episode in range(test_num):
        ...
        if done:
            if len(path_set) != 0:
                ...
                policy_network.compute_loss(np.reshape(state_batch, (-1, state_dim)), 0.1 * diverse_reward, action_batch)
